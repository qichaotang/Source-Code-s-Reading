...
void exit_with_help()
{
	...
	// 存储缩放规则train3.range，缩放结果存储与train3.scale
	"-r restore_filename : restore scaling parameters from restore_filename\n"  // 表示将缩放规则文件restore_filename 载入后按此缩放；
	// 加载缩放规则train3.range，使用该规则进行缩放，结果存储于train3.scale
	);
	exit(1);
}
...
#define max(x,y) (((x)>(y))?(x):(y))  // 宏定义，加上()，作为预防措施，防止替换时候，出BUG
...
{
	int i,index;
	FILE *fp, *fp_restore = NULL;  // fp：文件类型；fp_restore：恢复文件类型
	char *save_filename = NULL;  // 存储文件名
	char *restore_filename = NULL;  // 再次存储（恢复）文件名
	...
	for(i=1;i<argc;i++)
	{
		...
		{
			case 'l': lower = atof(argv[i]); break;  // 下界
			case 'u': upper = atof(argv[i]); break;  // 上界
			case 'y':
				y_lower = atof(argv[i]);
				++i;
				y_upper = atof(argv[i]);
	...
	if(!(upper > lower) || (y_scaling && !(y_upper > y_lower)))  // lower下界/upper上界输入有误
	{
		fprintf(stderr,"inconsistent lower/upper specification\n");
		exit(1);
	}
	
	if(restore_filename && save_filename)  // 同时使用-r,-s会有误
	{
...
#define SKIP_TARGET\  // 跳过目标集的空格
	while(isspace(*p)) ++p;\
	while(!isspace(*p)) ++p;

#define SKIP_ELEMENT\  // 跳过特征元素的空格与":"
	while(*p!=':') ++p;\
	++p;\
	while(isspace(*p)) ++p;\
	while(*p && !isspace(*p)) ++p;
	...
	if(restore_filename)  // 对数据进行文件的重新存储
	{
		int idx, c;
		...
		rewind(fp_restore);  // 将文件内部的指针重新指向一个流的开头函数名
	}
	...
	rewind(fp);  // 将文件内部的指针重新指向一个流的开头函数名
	...
	while(readline(fp)!=NULL)
	{
		...
		while(sscanf(p,"%d:%lf",&index,&value)==2)
		{
			...
			{
				feature_max[i]=max(feature_max[i],0);  // 特征最大值
				feature_min[i]=min(feature_min[i],0);  // 特征最小值
			}
...
void output(int index, double value)
{
	...
	// 跳过属性值
	if(feature_max[index] == feature_min[index])  // 表示属性值相等
		return;
	...
	if(value == feature_min[index])
		...
		value = lower + (upper-lower) * 
			...
			(feature_max[index]-feature_min[index]);  // 进行缩放
