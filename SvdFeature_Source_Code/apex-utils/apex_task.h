...
namespace apex_utils{
    ...
    inline int run_task( int argc , char *argv[] , ITask *tsk ){
        ...
        tsk->set_task( argv[1] );  // 此时argv[1]为 basicMF.conf 这个配置文件，设置此个basicMF任务的参数,apex_svd::SVDTrainTask类型
        // 此个被调用了!!!
        for( int i = 2 ; i < argc ; i ++ ){
            char name[256],val[256];
            if( sscanf( argv[i] ,"%[^=]=%[^\n]", name , val ) == 2 ){  // num_round=40 提取此个式子，则name="num_round", val="40"，此个式子值
                tsk->set_param( name , val );  // 设置tsk中参数name值，在此个例子中:设置SVDTrainTask类中，SVDTrainTask::set_param("num_round", "40")
		}   
        }
        tsk->run_task();  // 运行SVDTrainTask::run_task() 这个函数，执行训练任务
        return 0;
    } 
};
#endif
