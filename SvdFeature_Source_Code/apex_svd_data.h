 ...
 */
 // 此个文件表示 输入数据结构 对于SVDFeature 及其 加载数据接口
...
namespace apex_svd{  
    /*! 
     * \brief storage block of random order input,   // Random Order Input 随机序列输入
     *  in CSR format for sparse matrix, multiple lines are stored together in a block
     * \sa SVDFeatureCSR::Elem
     */
    struct SVDFeatureCSR{  // SVDFeatureCSR：表示数据格式的结构体,即整个矩阵结构体
        /*! 
         * \brief storage block of random order input, a single line of feature  // 一行特征
         */
        struct Elem{  // Elem：表示每列的结构体，即表示每条样本的向量结构体
            /*! \brief result label, rate or {0,1} for classification */
            float  label;
            /*! \brief number of nonzero global feature */
            int num_global;  // global feature
            /*! \brief number of nonzero user feature */
            int num_ufactor;  // user feature
            /*! \brief number of nonzero item feature */
            int num_ifactor;  // item feature
            /*! \brief array of global feature index */
            unsigned *index_global;  // global feature：全局特征-特征下标值表
            ...
            float *value_global;  // global feature：全局特征-特征值表
            ...
            /*! 
             * \brief set the data space of feature  // 设置特征的数据空间
            ...
            /*! 
             * \brief clone another copy of data  // 克隆-复制数据
         ...
         */
         // global feature, user feature, item feature, 3种特征
        int   *row_ptr;        
        ...
        inline Elem operator[]( int r ){
            Elem e;
            e.label = row_label[ r ];
            e.num_global   = row_ptr[ r * 3 + 1 ] - row_ptr[ r * 3 + 0 ];  // 为何是相减的呢？
        ...
        /*! 
         * \brief get a submatrix, with same number of column, but a subrange of rows  // 获取子矩阵，不过列数相等，取得子行数
...
namespace apex_svd{
     ...
     */
    class SVDFeatureCSRPage{  // 页的类
    public:
        /*!\brief constant for pagesize */
        static const int psize = 1 << 20;  // page size（页大小<存储空间大小>）
    private:
        /*!\brief pointer to the space */
        int *dptr;  // 指向整个数据矩阵空间的指针        
    public:
        /*!\brief constructor */
     ...
     *  that's too large to be fit into one block  // 数据块，信息块
     */    
    namespace svdpp_tag{  // 与枚举类型作用类似，感觉
        /*! 
