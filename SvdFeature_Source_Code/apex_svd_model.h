...
namespace apex_svd{
    ...
    namespace active_type{
         ...
         */        
        inline float cal_grad( float r, float pred, int type ){  // 计算出目标函数梯度
            switch( type ){
    ...
    struct SVDTypeParam{
        ...
        SVDTypeParam( void ){
            ...
            active_type = extend_type = variant_type = 0;  // extend_type 默认值为0，表示外部模型
        }
        /*! 
    ...
    struct SVDModel{  //
        ...
        SVDTypeParam  mtype;  // solver类型
        /*! \brief model parameters */ 
        SVDModelParam param;  // 模型参数
        /*! \brief whether the space is allocated */
        int space_allocated;  // 空间是否被分配
        /*! \brief common bias space for user/item feature */
        apex_tensor::CTensor1D ui_bias;  // user/item feature 的偏差空间
        /*! \brief common latent factor parameter for user/item feature */
        apex_tensor::CTensor2D W_uiset;  // user/item feature 的潜在因子      
        /*! \brief user bias */
        apex_tensor::CTensor1D u_bias;  // user 的偏差
        /*! \brief user latent factor */
        apex_tensor::CTensor2D W_user;  // user 的潜在因子
        /*! \brief item bias */
        apex_tensor::CTensor1D i_bias; // item 的偏差
        /*! \brief item latent factor */
        apex_tensor::CTensor2D W_item;  // item 的潜在因子
        /*! \brief global bias */
        apex_tensor::CTensor1D g_bias;  // global 的偏差
        /*! \brief user feedback bias */
        apex_tensor::CTensor1D ufeedback_bias;  // user feedback bias
        /*! \brief user feedback latent factor */
        apex_tensor::CTensor2D W_ufeedback;  // user feedback latent factor       
        /*! \brief constructor */
        SVDModel( void ){
            space_allocated = 0;
        }
        /*! \brief allocated space for a given model parameter */
        inline void alloc_space( void ){  // 根据模型参数分配空间
            {// allocate space for user/item factor
                ...
                if( param.common_latent_space == 0 ){  // common_latent_space 默认值为0
                ...
                apex_tensor::tensor::alloc_space( ui_bias );  // CTensor1D::alloc_space
                apex_tensor::tensor::alloc_space( W_uiset );  // CTensor2D::alloc_space
        ...
        inline void save_to_file( FILE *fo ) const{  //
            ...
            {// handle for common user/item latent space, make it compatible with previous format
                if( param.common_latent_space == 0 ){  // 写入文件中
         ...
         */ 
        // 使用随机数初始化模型相关参数
        inline void rand_init( void ){  // 模型参数的随机初始化
            ui_bias = 0.0f;
            g_bias  = 0.0f;
			// Debug
            param.base_score = active_type::calc_base_score( param.base_score, mtype.active_type );  // 基准预测-平均值作为预测值
            {// initialize ufactor  // ufactor
            ...
            // only need to initialize once in common latent space  // common latent space
            if( param.common_latent_space == 0 ){
                // initialize ifactor
                ...
                apex_tensor::tensor::sample_gaussian( W_iinit, param.i_init_sigma );  // 使用高斯分布初始化参数
                // fix for non negative weight
