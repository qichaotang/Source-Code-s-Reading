...
namespace apex_svd{
    ...
    class ParameterSet{  // 参数类，被用于进行详细的调参操作
    private:
        // weight decay
        std::vector<float>    wd;
        // bound for index 
    ...
    public :
        ...
        inline void set_param( const char *name, const char *val ){
            if( !strncmp( name, prefixA, prefix_lenA ) ){  // 在前prefix_lenA个子串进行比较，name与prefixA相等，即prefixA为name的前缀子串
                name += prefix_lenA;  // 后移，即去掉前缀
            }else{
                if( !strncmp( name, prefixB, prefix_lenB ) ){  // 即prefixB为name的前缀子串
                    name += prefix_lenB;
                }else return;
            }
...
namespace apex_svd{    
    class SVDFeature: public ISVDTrainer{  // 工具箱主要算法包，继承抽象类ISVDTrainer
    protected:
        int init_end;
        SVDModel model;  // SVDFeature 的默认模型
        SVDTrainParam param;  // SVDFeature 的默认训练参数
    protected:
    ...
    private:
        // data structure for detail weight decay tuning  // 对于详细的权值下降调节的数据结构
    ...
    public:
        ...
        virtual void set_param( const char *name, const char *val ){  // 模型相关参数设置
        ...
        virtual void init_model( void ){  // 通过设置默认初始化model
            // Debug--->
            model.alloc_space();  // 分配空间
            model.rand_init();  // 初始化参数,使用高斯分布初始化相关参数->W_uinit,W_iinit
        }
        // initialize trainer before training 
        virtual void init_trainer( void ){  // 初始化训练器
    ...
    private:
        inline void reg_global( const unsigned gid ){  // Reg global
        ...
        inline void reg_user( const unsigned uid ){  // Reg user
            // regularize factor 
        ...
        inline void reg_item( const unsigned iid ){  // Reg item
            CTensor1D w;           
        ...
        // do regularization
        inline void regularize( const SVDFeatureCSR::Elem feature, bool is_after_update ){  // regularize-正则化操作           
    ...
    private:
        inline double calc_bias( const SVDFeatureCSR::Elem &feature,
            ...
            if( model.param.no_user_bias == 0 ){
                ...
                sum += this->get_bias_svdpp();  // 无作用，就是加0
            }
             
            sum += this->get_bias_plugin( feature );  // 无作用，就是加0
        ...
        inline void update_no_decay( float err, const SVDFeatureCSR::Elem &feature  ){ 
            ...
            for( int i = 0; i < feature.num_ufactor; i ++ ){  //
                ...
                model.W_user[ uid ] += scale * tmp_ifactor;  // 更新 model.W_user 的值
            ...
            for( int i = 0; i < feature.num_ifactor; i ++ ){  //
                ...
                model.W_item[ iid ] += scale * tmp_ufactor;  // 更新的方式与公式上叙述的一样!!!               
    ...
    protected:
        inline float pred( const SVDFeatureCSR::Elem &feature ){  // 预测函数
            double sum = model.param.base_score + 
                this->calc_bias( feature, model.u_bias, model.i_bias, model.g_bias );  // 偏差
            
            this->prepare_tmp( feature );  //

            sum += apex_tensor::cpu_only::dot( tmp_ufactor, tmp_ifactor );  // 矩阵的点乘,此个式子就为与激活函数想结合的式子吧?即最后面的式子   !!!<继续Debug这里看看具体原因如何>        
            
            return active_type::map_active( (float)sum, model.mtype.active_type );  // 激活函数
        }

        inline void update_inner( const SVDFeatureCSR::Elem &feature, float sample_weight = 1.0f ){ 
            this->regularize( feature, false );  // 正则化操作，基准分类模型时候，无正则化操作，故在basic-MF模型操作中，无作用
            // Debug feature.label值看看，怎么全是0?
            float err = active_type::cal_grad( feature.label, this->pred( feature ), model.mtype.active_type ) * sample_weight;  // 计算出相应梯度
            this->update_no_decay( err, feature  ); //
            this->sample_counter ++;  // lazy decay 计数
            this->regularize( feature, true );  //
        }
    public:        
        virtual void update( const SVDFeatureCSR::Elem &feature ){  // 更新---模型训练时候            
            this->update_inner( feature );  // 根据每次特征值进行更新
        }
        virtual float predict( const SVDFeatureCSR::Elem &feature ){  // 预测---模型预测时候
            return this->pred( feature );
        }
        virtual void set_round( int nround ){  //
            if( param.decay_learning_rate != 0 ){  // 每次都为0么?
                apex_utils::assert_true( round_counter <= nround, "round counter restriction" );
				// Debug --->round_counter值看看，是否每次都发生改变
                while( round_counter < nround ){
                    param.learning_rate *= param.decay_rate;  // 通过执行多次模型，生成多个不同模型，主要就是改变了learning_rate，即迭代次数
                    round_counter ++;
                }
            }
        }
