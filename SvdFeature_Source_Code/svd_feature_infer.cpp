...
namespace apex_svd{
    ...
    private:
        ...
        public:
            ...
            inline void add_eval( float label, float pred, float scale = 1.0f ){  // 根据每个样例计算每个样例的误差
                double diff = (pred - label) * scale;
                sum_test += diff*diff;
                num_test += 1.0;            
            }
            inline void print_stat( FILE *fo, int iter ){  // 计算整个误差
                fprintf( fo, "%d\t%lf\n", iter, sqrt( sum_test/num_test )  );
            }
            inline void before_first(){  // 初始化该参数值
    ...
    private:
        inline void reset_default(){  // 初始化相关参数值
            this->step = 1;
            this->pred_model  = -1;  // 初始化模型ID为-1
            this->pred_binary = 0; 
            ...
            strcpy( name_pred  , "pred.txt" );  // 预测值文件
            strcpy( name_model_in_folder, "models" );  // 模型所有目录，但是在.conf配置文件中为"./"样例            
            strcpy( name_job, "" );
            strcpy( name_eval, "NULL" );       
            silent = 0; start = 0; end = INT_MAX;  // start初始化为0
            use_ranker = 0; num_item_set = 0;  // use_ranker初始化为0
    ...
    private:
        inline void init_model( int start ){
            ...
            if( use_ranker == 0 ){  // use_ranker初始化值为0，故执行此步骤
                svd_inferencer = create_svd_trainer( mtype );  // 创建svd模型，注意此个函数的构造
                svd_inferencer->load_model( fi );  // 加载事先训练的.model文件
                fclose( fi );
            }else{
        ...
        inline void configure_iterator( void ){
            if( mtype.format_type == svd_type::USER_GROUP_FORMAT ){
                ...
                this->itr_csr = create_csr_iterator( input_type );  // basic_MF为此种数据格式
            }        
        ...
        inline void init( void ){
            rmse_eval.init();  // 评价指标函数初始化
            this->init_model( start );  // start初始化为0,又是从第0个模型开始Debug
            this->configure_inferencer();  // 配置接口
            if( svd_inferencer != NULL ) svd_inferencer->init_trainer();  // 默认时候 SVDFeature 类进行初始化
            if( svd_ranker != NULL ) svd_ranker->init_ranker( num_item_set );
            this->configure_iterator();  //
            this->init_end = 1;
        }     
     
        inline void task_eval(){  // 执行模型认证任务
            FILE *fo = stdout;
            ...
            for( int iter = start; iter < end && this->load_model(iter); iter += step ){
                ...
                if( itr_csr != NULL ){  // itr_csr数据格式情况
                    itr_csr->before_first();
                    SVDFeatureCSR::Elem e;
                    while( itr_csr->next(e) ){  // 对每列数据进行预测                  
                        float p = svd_inferencer->predict( e );  // 模型值预测, SVDFeature::predict()
                        rmse_eval.add_eval( e.label, p, scale_score );
                    }                    
                }
                if( itr_plus != NULL ){  // itr_plus数据格式情况
                    SVDPlusBlock e;
                ...
                rmse_eval.print_stat( fo, iter );  // 第 iter 次迭代 结果写入内存
            }
    ...
    public:
        ...
        virtual void run_task( void ){            
            this->configure();  // 初始化
            this->init();
            if( this->pred_model >= 0 ){  // pred_model此个初始化值为-1?                
...
int main( int argc, char *argv[] ){
    apex_random::seed( 10 );
    apex_svd::SVDInferTask tsk;  // 模型纠正
    return apex_utils::run_task( argc, argv, &tsk );  // 此时tsk为apex_svd::SVDInferTask类型，使用pred=40，表示使用第40个模型进行预测，即 0040.model
}
